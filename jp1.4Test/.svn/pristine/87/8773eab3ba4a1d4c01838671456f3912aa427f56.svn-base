...
//				 }
			 	  ...
			 	  var start=new Date(searchParam.beginReportTime.replace("-", "/").replace("-", "/"));  
			 	  var end=new Date(searchParam.endReportTime.replace("-", "/").replace("-", "/"));  
	...
	function daysBetween(DateOne,DateTwo) {
		var cha=((DateOne.getTime()- DateTwo.getTime())/86400000);
		return Math.abs(cha);
	}
