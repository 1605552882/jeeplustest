...
import com.jeeplus.modules.sys.utils.AppUtil;
import com.jeeplus.modules.sys.utils.UserUtils;
import com.jeeplus.modules.sys.utils.VenustechUtils;

import cn.com.venustech.caserver.api.AuthenticationService;
import cn.com.venustech.caserver.api.entity.ResponseEntity;
import cn.com.venustech.caserver.common.util.MD5Tool;
import cn.com.venustech.caserver.model.Verification;
...
public class LoginController extends BaseController {
	...
	@RequestMapping(value = "${adminPath}/login/sendSms", method = RequestMethod.GET)
	public String sendSms(HttpServletRequest request, HttpServletResponse response, Model model) {
		String username = "cj_zhangc8";
		String password = "Skywin_zhangch1";
		String passwd = MD5Tool.MD5Encrypt(String.valueOf(AppUtil.passwordReplace(password.toString())));
		Verification ver = new Verification();
		ver.setType("APP_LOGIN");
		ver.setName(username);
		ver.setPassword(passwd);
		ResponseEntity<Verification> entity = AuthenticationService.getInstance()
				.getVerification(VenustechUtils.getAPIAuth(), ver);
		System.out.println("发送短信编码===" + entity.getEntity().getResponseCode());
		System.out.println("发送短信结果===" + entity.getEntity().getResponseRemark());
		if (entity.getEntity().getResponseCode().equals("0")) {
			model.addAttribute("success", true);
			model.addAttribute("message", "短信发送成功");
		} else if (entity.getEntity().getResponseCode().equals("1015")) { // 补充手机号码
			model.addAttribute("success", false);
			model.addAttribute("type", "1");
		} else {
			model.addAttribute("success", false);
			model.addAttribute("message", entity.getEntity().getResponseRemark());
		}
		return "modules/sys/login/sysLogin";
	}
