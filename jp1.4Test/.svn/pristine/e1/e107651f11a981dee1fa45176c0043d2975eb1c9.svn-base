...
$(document).ready(function() {
	var d = new Date($("#beginDcreatetime").val());
	$("#beginDcreatetime").val(dateFormat("YYYY-mm-dd HH:MM:SS", d));
	var d = new Date($("#endDcreatetime").val());
	$("#endDcreatetime").val(dateFormat("YYYY-mm-dd HH:MM:SS", d));
...
function newtable(sbillno) {
	  jp.post("${ctx}/overtimedocument/overtimedocument/newtable",{sbillno:sbillno},function(data){
		  if(data.success){
			  ...
			  jp.openTab("${ctx}/documentdetect/documentarydetails/samplform?id="+id,"工单统计-工单明细", false);
		  }
	  });
}
