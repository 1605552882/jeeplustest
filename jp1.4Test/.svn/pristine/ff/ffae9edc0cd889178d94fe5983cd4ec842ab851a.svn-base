...
$(document).ready(function() {
	...
	$('#errordocumentTable').bootstrapTable({
		  ...
		  //请求方法
               ...
               url: "${ctx}/errordocument/errordocument/data1",
...
function newtable(sbillno, remarks , checkrule) {
		...
		jp.post("${ctx}/overtimedocument/overtimedocument/newtable",{sbillno:sbillno},function(data){
			if(data.success){
				...
				jp.openTab("${ctx}/documentdetect/documentarydetails/samplform?id="+detailId + "&rule="+checkrule ,"本部门派错-工单明细", false);
			}
		});
	}
  ...
  function edit(){
	...
	function exportAll(){
		 ...
		 form.attr('action', "${ctx}/errordocument/errordocument/export1");
		 $('body').append(form);
